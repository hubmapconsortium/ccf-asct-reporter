...
export class ControlPaneComponent implements OnInit {
  ...
  constructor(
    ...
    this.filteredProteins$.subscribe(() => {
      ...
      const filteredProtiens = this.store.selectSnapshot(
        SheetState.getFilteredProtiens
      );
      if (data.length) {
  ...
  showAllAS() {
    this.store.dispatch(new ToggleShowAllAS()).subscribe((states) => {
      ...
      this.store.dispatch(
        new FetchSelectedOrganData(sheet, selectedOrgans, omapSelectedOrgans)
      );
    });
  }
