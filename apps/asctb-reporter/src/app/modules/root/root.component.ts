...
export class RootComponent implements OnInit, OnDestroy {
  ...
  exportVis(option: string) {
    ...
    const selectedOrgans = this.store.selectSnapshot(
      SheetState.getSelectedOrgans
    );
      ...
      this.sheetService
        ...
        .subscribe((graphData: GraphData) => {
          ...
          downloadAnchorNode.setAttribute(
            'download',
            `asct+b_graph_data_${sn}_${dt}` + '.json'
          );
    ...
    if (this.bimodalConfig.BM.type != 'Protein') {
      ...
      setTimeout(() => {
        this.snackbarRef = this.snackbar.open(
          'OMAP Detected, Do you want to filter out only Proteins ' +
            'from the available BioMarkers?',
          'Filter',
          config
        );
