...
export class SheetState {
  ...
  async fetchSelectedOrganData(
    ...
    patchState({
      allOmapOrgans: this.omapSheetConfig.map(
        (organObject) => organObject.name
      ),
    });
  ...
  private omapFiltering(
    ...
    if (event.organsOnly) {
      ...
      //Filtered List
      newSelectedOrgans = this.organFiltering(
        currentlySelectedOrgans,
        sheetState.omapSelectedOrgans
      );
      const newSelectedBeforeOrgans = currentlySelectedOrgans.map((organ) =>
        this.convertOrganToBeforeFilterFormat(
          organ,
          !newSelectedOrgans.includes(organ)
        )
      );
      this.store.dispatch(
        new UpdateSelectedOrgansBeforeFilter(newSelectedBeforeOrgans)
      );
  ...
  private organFiltering(selectedOrgans, omapSelectedOrgans): string[] {
    ...
    if (omapSelectedOrgans.length > 0 && omapSelectedOrgans[0] != '') {
      ...
      const filteredOmapOrgans = this.omapSheetConfig
        .filter((organ) =>
          selectedOmapOrgans.includes(organ.name.toLowerCase())
        )
  ...
  private convertOrganToBeforeFilterFormat(
    organ: string,
    filteredOut: boolean
  ): selectedOrganBeforeFilter {
        ...
        tap((res: ResponseData) => {
          ...
          if (sheet.name === 'example') {
            ...
            this.ga.event(
              ...
              `Adding sheet link or file to Playground : ${JSON.stringify(
                gaData
              )}`,
              0
            );
  ...
  fetchValidOmapProtiens({
    ...
    forkJoin(requests$).subscribe((allresults) => {
      ...
      const filteredProtiens =
        new Set([...existingProtiens].filter((i) => allOmapProtiens.has(i))) ??
        [];
      patchState({
