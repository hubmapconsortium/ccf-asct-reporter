...
export class OmapDataTransformer {
  ...
  private transformOmapData(): string[][] {
    ...
    asctbConverted.push(
      ...this.data.slice(0, this.headerRow),
      this.createNewHeaderRow(),
      ...this.createData()
    );
  ...
  private createNewHeaderRow(): string[] {
    const maxProteins = this.data
      .slice(this.headerRow + 1)
      .map((subArr) => subArr[0].split(',').length);
  ...
  private createData(): string[][] {
    const dataObject: Record<string, string>[] =
      this.createMapOfOldColumnsAndValues();
    // Decides whether to take organs from table or constants
    const organColumnsPresent = ['organ', 'organ_uberon'].every((column) =>
      this.columns.includes(column)
    );
    const organ =
      OMAP_ORGAN[this.metaData.data_doi as string] ?? OMAP_ORGAN.default;
    if (!(this.isLegacyOmap || organColumnsPresent)) {
      this._warnings.add(
        'WARNING: Organ Columns Missing. Adding default Organ Columns'
      );
    }
    if (this.isLegacyOmap && !OMAP_ORGAN[this.metaData.data_doi as string]) {
      this._warnings.add(
        'WARNING: DOI mapping not present; Adding default Organ Columns.'
      );
    }
    ...
    dataObject.forEach((data) => {
      ...
      if (
        !(
          uniprots.length === hgncIds.length &&
          hgncIds.length === targetNames.length
        )
      ) {
        this.warnings.add(
  ...
  private createMapOfOldColumnsAndValues(): Record<string, string>[] {
    ...
    this.data.slice(this.headerRow + 1).forEach((subArr, index) => {
      const keyValuePairs = this.columns.reduce(
        (acc: Record<string, string>, key, i) => {
          acc[key] = subArr[i];
          return acc;
        },
        {}
