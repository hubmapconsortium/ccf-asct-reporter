...
function setData(
  ...
  if (column.length > 1) {
    ...
    if (!arrayName) {
      warnings.add(
        `WARNING: unmapped array found ${originalArrayName} (Code ${WarningCode.UnmappedData})`
      );
    }
    ...
    if (column.length === 2) {
      ...
      if (arrayIndex >= 0 && fieldName) {
        if (arrayIndex >= objectArray.length) {
          warnings.add(
            `WARNING: blank cells likely found in column: ${column.join(
              '/'
            )}, row: ${row.rowNumber}`
          );
        }
        ...
        if (arrayIndex < objectArray.length) {
          ...
          if (objectArray[arrayIndex]) {
            ...
            warnings.add(
              `WARNING: bad column: ${column.join('/')} (Code ${
                WarningCode.BadColumn
              })`
            );
          }
        }
  ...
  return metadataRows.reduce(
    (
      ...
      if (!metadataIdentifier) {
        warnings.add(
          `WARNING: Metadata Key missing found at Row: ${rowNumber + 3} (Code ${
        ...
        warnings.add(
          `WARNING: Metadata Value missing found at Row: ${
            rowNumber + 3
          } (Code ${WarningCode.UnmappedMetadata})`
        );
      }
      ...
      if (!metadataKey) {
        ...
        warnings.add(
          `WARNING: unmapped metadata found ${metadataIdentifier} at Row: ${
            rowNumber + 3
          } (Code ${WarningCode.UnmappedMetadata})`
        );
      }
...
function validateHeaderRow(
  ...
  headerData.forEach((value) => {
    ...
    if (value.length === 3) {
      ...
      const col0Warnings =
        value[0].trim().length === 0 || !arrayNameMap[value[0].toUpperCase()];
      const col1Warnings =
        value[1].trim().length === 0 || Number.isNaN(parseInt(value[1]));
      const col2Warnings =
        value[2].trim().length === 0 || !objectFieldMap[value[2]];
  ...
  if (isId) {
    ...
    if (nameValue) {
      if (!idValue) {
        ...
        warnings.add(
          `WARNING: Missing Uberon/CL ID at Column: ${colName}, Row: ${
            row.rowNumber + 1
          } (Code ${WarningCode.MissingCTorAnatomy})`
        );
      } else if (!labelValue) {
        const colName = columnIndexToName(index - 1);
        warnings.add(
          `WARNING: Missing RDFS Label for ID ${idValue} at Column: ${colName}, Row: ${
            row.rowNumber + 1
          } (Code ${WarningCode.MissingCTorAnatomy})`
        );
      }
      if (
        column.join('/') == 'CT/1/ID' &&
        (!idValue || !idValue.startsWith('CL:'))
      ) {
        const colName = columnIndexToName(index);
        warnings.add(
          `WARNING: CT/1/ID is not a CL ID (required) at Column: ${colName}, Row: ${
            row.rowNumber + 1
          } (Code ${WarningCode.NoIdInCT1})`
        );
      }
    }
  }
...
export function makeASCTBData(data: string[][]): ASCTBData | undefined {
  ...
  if (header[0] === LEGACY_OMAP_HEADER_FIRST_COLUMN) {
    ...
    throw new Error(
      `Header row, first column should be : ${ASCT_HEADER_FIRST_COLUMN} or ${OMAP_HEADER_FIRST_COLUMN}`
    );
  }
}
